...
def run(config):
  ...
  config['n_classes']    = utils.nclass_dict[config['dataset']] * config['cluster_per_class']
  ...
  if config['is_encoder']:
    ...
    config['lambda_encoder']       = config['resolution']**2 * config['dataset_channel']
  ...
  state_dict = {'itr': 0, 'epoch': 0, 'save_num': 0, 'save_best_num': 0,
                'best_IS': 0, 'best_FID': 999999, 'best_error_rec': 99999,'config': config}
  ...
  if config['which_train_fn'] == 'GAN':
    train = train_fns.GAN_training_function(G, D, GD, Prior, ema, state_dict, config,
                                            losses.Loss_obj(**config), 
  ...
  for epoch in range(state_dict['epoch'], config['num_epochs']):    
    ...
    for i, (x, y) in enumerate(pbar):
      ...
      if (config['sv_log_interval'] > 0) and (not (state_dict['itr'] % config['sv_log_interval'])):
        ...
        if config['is_encoder']:
          train_log.log(itr=int(state_dict['itr']), **{**utils.get_SVs(E, 'E')})
      ...
      if not (state_dict['itr'] % config['save_every']):
        if config['G_eval_mode']:
          ...
          if config['ema']:
            G_ema.eval()
        train_fns.save_and_sample(G, D, G_ema, Prior, fixed_z, fixed_y,
                                  state_dict, config, experiment_name, None if not config['is_encoder'] else E)


      if not (state_dict['itr'] % config['test_every']) and config['is_encoder']:
        if not config['prior_type'] == 'default':
        ...
        if config['n_classes'] == 10:
          utils.reconstruction_sheet(GE, classes_per_sheet=utils.classes_per_sheet_dict[config['dataset']],
                                   num_classes = config['n_classes'], samples_per_class = 20, parallel = config['parallel'],
                                   samples_root= config['samples_root'], experiment_name = experiment_name,
                                   folder_number =state_dict['itr'], dataloader= dataloader_noaug, device = device,
      ...
      if not (state_dict['itr'] % config['test_every']):
        if config['G_eval_mode']:
          ...
          if not config['prior_type'] == 'default':
            Prior.eval()
          G.eval()
        train_fns.test(G, D, G_ema, Prior, state_dict, config, sample, get_inception_metrics, experiment_name, 
                        test_log, None if not config['is_encoder'] else E, None if config['prior_type'] == 'default' else (test_acc, test_acc_iter, error_rec, p_mse, p_lik) )

      if not (state_dict['itr'] % config['test_every']):
        utils.create_curves(train_metrics_fname, plot_sv = False, prior_type = config['prior_type'], is_E = config['is_encoder'])
