...
def prepare_parser():
  ...
  parser.add_argument(
    '--is_disc_cond_y', action='store_true', default=False,
    help='is discriminator conditioned on y? (default: %(default)s)')
  parser.add_argument(
    '--is_encoder', action='store_true', default=False,
    help='is encoder? (default: %(default)s)')
  parser.add_argument(
    '--is_enc_shared', action='store_true', default=False,
    help='is encoder shared? (default: %(default)s)')
  parser.add_argument(
    '--is_loss3', type=float, default= 0.0,
    help='is using loss 3? (default: %(default)s)')
  parser.add_argument(
  ...
  parser.add_argument(
    '--shared_hier', action='store_true', default=False,
    help='Use shared hierarchical z in G? (default: %(default)s)')
  parser.add_argument(
  ...
  parser.add_argument(
    '--num_E_steps', type=int, default=1,
    help='Number of E steps per G step (default: %(default)s)')
  parser.add_argument(
  ...
  parser.add_argument(
    '--update_GMM_every_n', type=int, default=1,
    help='iteration to update Prior GMM? (default: %(default)s)')
  parser.add_argument(
  ...
  ### Bookkeping stuff ###  
  parser.add_argument(
    '--is_not_rec', action='store_true', default=False,
    help='Is Rec at sample/test time? '
         '(default: %(default)s)')
  parser.add_argument(
  ...
  parser.add_argument(
    ...
    help='Use a hash of the experiment name instead of the full config '
         '(default: %(default)s)')
  parser.add_argument(
    '--num_batch_prior_acc', type=int, default=200,
    help='num batch prior acc (default: %(default)s)')
  parser.add_argument(
    '--num_iter_prior_acc', type=int, default=50,
    help='num iteration of test sample (default: %(default)s)')
  ...
  ### Encoder stuff ###
  parser.add_argument(
    '--is_E', action='store_true', default=False,
    help='is Encoder utilized? (default: %(default)s)')
  parser.add_argument(
    '--E_ortho', type=float, default=0.0,
  ...
  parser.add_argument(
    '--is_latent_detach', action='store_true', default=False,
    help='is latent detach when optimized with encoder? (default: %(default)s)')
  parser.add_argument(
...
# Convenience dicts
dset_dict = {'C10': dset.CIFAR10, 'C100': dset.CIFAR100,
             'MNIST': dset.MNIST,'FMNIST': dset.FashionMNIST,
             'OMNIGLOT': dset.OMNIGLOT, 'C20': dset.CIFAR100}
imsize_dict = {'C10': 32, 'C100': 32, 'C20': 32,
               'MNIST': 32, 'SVHN': 32, 'SVHN_gray': 32,
               'FMNIST': 32, 'OMNIGLOT': 32, 'USPS': 32, 'ZTF33': 32, 'ZTF17': 16, 'ZTF32': 32, 'ZTF32_gray': 32}
root_dict = {'C10': 'cifar', 'C100': 'cifar', 'C20': 'cifar100',
             'MNIST': 'mnist', 'FMNIST': 'fmnist', 'OMNIGLOT': 'omniglot'}
nclass_dict = {'C10': 10, 'C100': 100, 'C20': 20,
               'MNIST': 10, 'FMNIST': 10, 'OMNIGLOT': 100}
# Number of classes to put per sample sheet               
classes_per_sheet_dict = {'C10': 10, 'C100': 100, 'C20': 20,
                          'MNIST': 10,
                          'FMNIST': 10, 'OMNIGLOT': 100}
#number of channel per datatset
channel_dict = {'C10': 3, 'C100': 3, 'C20': 3,
               'MNIST': 1, 'FMNIST': 1, 'OMNIGLOT': 1}
...
def _cifar100_to_cifar20(target):
  ...
  _dict = \
    {0: 4, 1: 1, 2: 14, 3: 8, 4: 0, 5: 6, 6: 7, 7: 7, 8: 18, 9: 3, 10: 3, 11: 14, 12: 9, 13: 18, 14: 7, 15: 11, 16: 3, 17: 9, 18: 7, 19: 11,20: 6,
     21: 11, 22: 5, 23: 10, 24: 7, 25: 6, 26: 13, 27: 15, 28: 3, 29: 15, 30: 0, 31: 11, 32: 1, 33: 10, 34: 12, 35: 14, 36: 16, 37: 9,38: 11,39: 5,40: 5,
     41: 19,42: 8,43: 8,44: 15,45: 13,46: 14,47: 17,48: 18, 49: 10, 50: 16, 51: 4, 52: 17, 53: 4, 54: 2, 55: 0, 56: 17, 57: 4, 58: 18, 59: 17, 60: 10,
     61: 3, 62: 2, 63: 12, 64: 12, 65: 16, 66: 12, 67: 1,68: 9,69: 19,70: 2,71: 10,72: 0, 73: 1, 74: 16, 75: 12, 76: 9, 77: 13, 78: 15, 79: 13, 80: 16,
     81: 19, 82: 2, 83: 4, 84: 6, 85: 19, 86: 5, 87: 5, 88: 8, 89: 19, 90: 18, 91: 1, 92: 2, 93: 15, 94: 6, 95: 0, 96: 17, 97: 8, 98: 14, 99: 13}
  return _dict[target]
...
class TransformTwice:
    def __init__(self, transform, transform_aug):
        self.transform     = transform
        self.transform_aug = transform_aug 

    def __call__(self, inp):
        out1 = self.transform(inp)
        out2 = self.transform_aug(inp)
        return out1, out2
...
def get_data_loaders(dataset, data_root=None, augment=False, batch_size=64, 
                     ...
                     num_epochs=500, use_multiepoch_sampler=False, equally_sample = False,
                     concat = False, **kwargs):
  ...
  if dataset in ['MNIST', 'FMNIST', 'OMNIGLOT']:
    norm_mean = [0.5]
    norm_std  = [0.5]
  ...
  if 'hdf5' in dataset:
    ...
    if augment:
      ...
      if dataset in ['C10', 'C100', 'C20']:
        train_transform = [transforms.RandomCrop(32, padding=4),
                           transforms.RandomHorizontalFlip()]
      elif dataset in ['MNIST', 'FMNIST', 'OMNIGLOT']:
        train_transform = [transforms.RandomCrop(32, padding=4),
                           transforms.RandomHorizontalFlip(),
      ...
      if dataset in ['C10', 'C100', 'C20', 'OMNIGLOT']:
        train_transform = []
    ...
    list_transform_normal = train_transform + [
                            transforms.ToTensor(),
  ...
  train_set = which_dataset(root=data_root, transform=train_transform, target_transform = target_transform,
                            load_in_mem=load_in_mem, **dataset_kwargs)

  if concat and not (dataset in ['OMNIGLOT']):
    test_set = which_dataset(root=data_root, train = False, transform=train_transform, target_transform = target_transform,
                            load_in_mem=load_in_mem, **dataset_kwargs)
  ...
  if use_multiepoch_sampler:
    ...
    if not equally_sample:
      ...
      train_loader = DataLoader(train_set, batch_size=batch_size,
                              sampler = BalancedBatchSampler(train_set, torch.tensor(train_set.labels)), **loader_kwargs)
...
def save_weights(G, D, state_dict, weights_root, experiment_name, 
                 name_suffix=None, G_ema=None, E = None, Prior = None):
  ...
  if Prior is not None:
    torch.save(Prior.state_dict(),
                '%s/%s.pth' % (root, join_strings('_', ['Prior', name_suffix])))
    torch.save(Prior.optim.state_dict(),
                '%s/%s.pth' % (root, join_strings('_', ['Prior_optim', name_suffix])))

# Load a model's weights, optimizer, and the state_dict
def load_weights(G, D, state_dict, weights_root, experiment_name, name_suffix=None,
                 G_ema=None, strict=True, load_optim=True, E = None, Prior = None):
  root = '/'.join([weights_root, experiment_name])
  if name_suffix:
  ...
  if E is not None:
    E.load_state_dict(
      torch.load('%s/%s.pth' % (root, join_strings('_', ['E', name_suffix]))),
      strict=strict)
    if load_optim:
  ...
  if Prior is not None:
    Prior.load_state_dict(
      torch.load('%s/%s.pth' % (root, join_strings('_', ['Prior', name_suffix]))),
      strict=strict)
    if load_optim:
      Prior.optim.load_state_dict(
        torch.load('%s/%s.pth' % (root, join_strings('_', ['Prior_optim', name_suffix]))))
...
def progress(items, desc='', total=None, min_delay=0.1, displaytype='s1k'):
  ...
  print("\r%s%d/%d (100.00%%) (took %d:%02d)" % ((desc, total, total) +
                                                   divmod(t_total, 60)))
''' Calculate accuracy'''
def obtain_latents(G_E, dataloader, device, D_fp16, config):
  #import pdb
  #pdb.set_trace()
  total_latent = []
  total_y_pred = []
  ...
  for x, y in dataloader[0]:
    ...
    for step_index in range(config['num_D_steps']):
      for accumulation_index in range(config['num_D_accumulations']):
        ...
        total_latent.append(o[0].data.cpu().numpy())
  ...
  #pdb.set_trace()
  return total_latent, total_y, total_y_pred

# Sample function for use with inception metrics
def sample(G, Prior, config):
  with torch.no_grad():
    z_, y_ = Prior.sample_()
    if config['parallel']:
...
def sample_sheet(G, Prior, classes_per_sheet, num_classes, samples_per_class, parallel,
                 samples_root, experiment_name, folder_number, transpose = False, num_rep = 1, additional_name = '' ):
  z_ = Prior.obtain_z()
  ...
  for i in range(num_classes // classes_per_sheet):
    ...
    for j in range(samples_per_class):
      if (z_ is not None) and hasattr(z_, 'sample_') and classes_per_sheet <= z_.size(0):
        z_.sample_()
        latent = Prior.obtain_latent_from_z_y(z_[:classes_per_sheet], y)
      else:
      ...
      with torch.no_grad():
        if parallel:
          o = nn.parallel.data_parallel(G, (latent, G.shared(y)))
        else:
          o = G(latent, G.shared(y))
    ...
    if not transpose:
      ...
      out_ims = torch.stack(ims, 1).reshape(classes_per_sheet * num_rep, 
                                    samples_per_class//num_rep, ims[0].shape[1], 
                                    ims[0].shape[2], ims[0].shape[3]).transpose(1,0).reshape(-1, 
                                    ims[0].shape[1], ims[0].shape[2], ims[0].shape[3]).data.float().cpu()
    # The path for the samples
    samples_name = 'samples' + additional_name
    image_filename = '%s/%s/%d/%s%d.jpg' % (samples_root, experiment_name,
                                                 folder_number, samples_name, i)

    if not transpose:
      torchvision.utils.save_image(out_ims, image_filename,
                                 nrow=samples_per_class//num_rep, normalize=True)
    else:
      torchvision.utils.save_image(out_ims, image_filename,
                                 nrow=classes_per_sheet*num_rep, normalize=True)


# Sample function for sample sheets
def reconstruction_sheet(G_E, classes_per_sheet, num_classes, samples_per_class, parallel,
                 samples_root, experiment_name, folder_number, dataloader, device, D_fp16, config):
  # Prepare sample directory
  #import pdb
  #pdb.set_trace()
  if not os.path.isdir('%s/%s' % (samples_root, experiment_name)):
  ...
  counter_of_classes = torch.zeros(num_classes)
  torch_eye          = torch.eye(num_classes)
  total_data         = []
  total_rec          = []
  total_label        = []
  # reconstruction
  for x,y in dataloader[0]:
    ...
    for step_index in range(config['num_D_steps']):
      for accumulation_index in range(config['num_D_accumulations']):
        ...
        with torch.no_grad():
          if parallel:
            latent_mu, _ = nn.parallel.data_parallel(G_E.E, x_counter)
          else:
            latent_mu, _ = G_E.E(x_counter)
          y_pred       = G_E.P.latent_classification(latent_mu)
          if parallel:
            o     = nn.parallel.data_parallel(G_E.G, (latent_mu ,G_E.G.shared(y_pred.argmax(1).long())))
          else: 
            o     = G_E.G(latent_mu, G_E.G.shared(y_pred.argmax(1).long()))
          #x_pred               = self.G(latent_mu, self.G.shared(y_pred.argmax(1).long()))
          #if parallel and False:
          #  o = nn.parallel.data_parallel(G_E, x_counter)
          #else:
          #  o = G_E(x_counter)
        o_cpu = o.data.cpu()
        x_cpu = x_counter.data.cpu()
        y_cpu = y_counter.data.cpu()
        total_data.append(x_cpu)
        total_rec.append(o_cpu)
        total_label.append(y_cpu)
        counter_of_classes += torch_eye[y_counter].sum(0)
        if min(counter_of_classes) >= samples_per_class:
          break
        counter += 1
  total_data  = torch.cat(total_data, 0)
  total_rec   = torch.cat(total_rec,  0)
  ...
  for i in range(num_classes // classes_per_sheet):
    ...
    for j in range(classes_per_sheet):
      index = total_label == j
      ims.append(total_data[index][:samples_per_class])
      ims.append(total_rec[index][:samples_per_class])
    # This line should properly unroll the images
    out_ims = torch.stack(ims, 1).view(-1, ims[0].shape[1], ims[0].shape[2], 
                                       ims[0].shape[3]).data.float().cpu()
    # The path for the samples
    image_filename = '%s/%s/%d/reconstruction%d.jpg' % (samples_root, experiment_name,
                                                 folder_number, i)
    torchvision.utils.save_image(out_ims, image_filename,
                                 nrow=classes_per_sheet*2, normalize=True)
...
def interp_sheet(G, Prior, num_per_sheet, num_midpoints, num_classes, parallel,
  ...
  with torch.no_grad():
    if parallel:
      out_ims = nn.parallel.data_parallel(G, (Prior.obtain_latent_from_z_y(zs, ys), ys)).data.cpu()
    else:
      out_ims = G(Prior.obtain_latent_from_z_y(zs, ys), ys).data.cpu()
...
def name_from_config(config):
  ...
  'seed%d' % config['seed'],
  'ccat' if config['concat'] else None,
  'Gch%d' % config['G_ch'],
  'Dch%d' % config['D_ch'],
  ...
  'nEs%d' % config['num_E_steps'] if config['num_E_steps'] > 1 else None,
  ...
  'GMMinit%s' % config['GMM_init'] if not config['GMM_init'] == 'ortho' else None,
  ...
  'SH' if config['shared_hier'] else None,
  'ema' if config['ema'] else None,
  'p%s' % config['prior_type'] if not config['prior_type'] == 'default' else None,
  'z_dtch' if config['is_latent_detach'] else None,
  'is_E' if config['is_encoder'] else None,
  'Eortho%2.1e' % config['E_ortho'] if config['E_ortho'] > 0.0 else None,
  'loss%s' % config['loss_type'] if not config['loss_type'] == 'hinge_dis' else None,
  'std%3.2f' % config['sigma'] if config['prior_type'] == 'GMM' else None,
  'is_D_y' if config['is_disc_cond_y'] else None,
  'E_nsh' if (not config['is_enc_shared'] and config['is_encoder']) else None,
  'UGE%d' % config['update_GMM_every_n'] if config['update_GMM_every_n'] > 1 else None,
  '%s' % config['optimizer_type'] if config['optimizer_type'] != 'adam' else None,
  'wd%2.1e' % config['weight_decay'] if config['weight_decay'] != 5e-4 else None,
  'L3_%3.3f' % config['is_loss3'] if config['is_loss3'] != 0 else None,
  'cpc%d' % config['cluster_per_class'],
...
class sampler_obj(object):
    def __init__(self, G_batch_size, dim_z, n_classes, device, G_fp16 ):
      z_, y_  = prepare_z_y(G_batch_size, dim_z, n_classes, device=device,
                                  fp16= G_fp16) 
      self.z_  = z_
      self.y_  = y_
    def sample(self):
        return self.z_.sample(), self.y_.sample()
...
def accumulate_standing_stats(net, Prior, nclasses, num_accumulations=16):
  initiate_standing_stats(net)
  initiate_standing_stats(Prior)
  net.train()
  Prior.train()
...
def accumulate_standing_stats_E(Enc, Prior, dataloader, device, config):
  initiate_standing_stats(Enc)
  initiate_standing_stats(Prior)
  Enc.train()
  Prior.train()
  total_counter = 0
    ...
    for step_index in range(config['num_D_steps']):
      for accumulation_index in range(config['num_D_accumulations']):
        if config['D_fp16']:
          ...
          x_counter, y_counter = x[counter].to(device), y[counter].to(device)
        with torch.no_grad():
          latent_mu, _ = Enc(x_counter)
          y_pred         = Prior.latent_classification(latent_mu)
        counter += 1
        total_counter += 1
        if total_counter == config['num_standing_accumulations']:
          break
  # Set to eval mode
  Enc.eval()
  Prior.eval()
...
def read_log(path):
    key      = []
    value    = []
    with open(path, 'r') as log_data:
        for line in log_data:
            split = line.split(':')
            if len(split) == 1:
                break
            key.append(int(split[0]))
            value.append(float(split[1].strip('\n')))
    return np.array(key), np.array(value)

def plot_sv_net(key, path):
    files = [file for file in sorted(os.listdir(path)) if key in file and 'sv' in file and '.log' in file]
    ...
    for i, file in enumerate(files):
        iterations, values = read_log('%s/%s'%(path, file))
        ax1.plot(iterations, values, label= str(file.strip('log')), color=cmap(i/len(files)))
        ax1.legend()

    ax1.set_xlabel('Iteration',  fontsize=12)
    ax1.set_ylabel('Network ' + str(key),  fontsize=12)
    
    #colormap = plt.cm.gist_ncar #nipy_spectral, Set1,Paired   
    #colors = [colormap(i) for i in np.linspace(0, 1,len(ax1.lines))]
    #for i,j in enumerate(ax1.lines):
    #    j.set_color(colors[i])
    plt.savefig('%s/Network_%s'%(path, key))
    plt.close(fig1)


def create_curves(path, plot_sv = False, prior_type = 'default', is_E = False):
    metrics = [key for key in sorted(os.listdir(path)) if not 'sv' in key and '.log' in key]
    fig1 = plt.figure(figsize = (12,12))
    ax1 = fig1.add_subplot(111)
    cmap = plt.get_cmap('jet')

    for i,keys in enumerate(metrics):
        iterations, values = read_log('%s/%s'%(path, keys))
        name = keys.strip('log')
        ax1.plot(iterations, values.clip(-1, 4), label= str(name), color=cmap((i + 0.5)/(len(metrics))))
        ax1.legend()
    ax1.set_xlabel('Iteration',  fontsize=12)
    ax1.set_ylabel( 'Loss',  fontsize=12)
    plt.savefig('%s/Loss'%path)
    plt.close(fig1)
    ...
    if not prior_type == 'default':
        metrics = [key for key in sorted(os.listdir(path)) if not 'sv' in key and '.log' in key and 'P' in key]
        for i,keys in enumerate(metrics):
            fig1 = plt.figure(figsize = (12,12))
            ax1 = fig1.add_subplot(111)
            cmap = plt.get_cmap('jet')
            ...
            ax1.legend()
            ax1.set_xlabel('Iteration',  fontsize=12)
            ax1.set_ylabel( '%s' % name,  fontsize=12)
            plt.savefig('%s/%s'% (path, name) )
            plt.close(fig1)

    if is_E:
        metrics = [key for key in sorted(os.listdir(path)) if not 'sv' in key and '.log' in key and 'E' in key]
        for i,keys in enumerate(metrics):
            fig1 = plt.figure(figsize = (12,12))
            ax1 = fig1.add_subplot(111)
            cmap = plt.get_cmap('jet')
...
def read_IS_FID(path):
    ...
    with open(path, 'r') as log_data:
        for line in log_data:
            ...
            iters.append(line_dict['itr'])
            IS.append(line_dict['IS_mean'])
            FID.append(line_dict['FID'])
    return np.array(iters), np.array(IS), np.array(FID)

def plot_IS_FID(path):
    iters, IS, FID = read_IS_FID('%s%s'%(path, '_log.jsonl'))
    fig1 = plt.figure(figsize = (12,12))
    ax1 = fig1.gca()
    ax1.plot(iters, IS)
    ax1.set_xlabel('Iteration',  fontsize=12)
    ax1.set_ylabel( 'IS',  fontsize=12)
    plt.rc('grid', linestyle="--", color='black')
    ax1.grid()
    plt.savefig('%s/%s'% (path, 'IS score') )
    plt.close(fig1)
    fig1 = plt.figure(figsize = (12,12))
    ax1 = fig1.gca()
    ax1.plot(iters, FID)
    ...
    plt.rc('grid', linestyle="--", color='black')
    ax1.grid()
    plt.savefig('%s/%s'% (path, 'FID score') )
    plt.close(fig1)
