...
def prepare_parser():
  ...
  parser.add_argument(
    '--experiment_root', type=str, default='weights',
...
def run(config):
  ...
  fname = '%s/%s/%s.npz' % (config['experiment_root'], config['experiment_name'], samples_name)
  ...
  if not os.path.isfile('%s/%s/%s.json' % (config['experiment_root'], config['experiment_name'], json_metric_name)):
    metric_dict = {}
    metric_dict['inception_mean' ]  = float(inc_mean)
    metric_dict['inception_std'  ]  = float(inc_std)
